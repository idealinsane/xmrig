...
public:
    ...
    void __attribute__((__annotate__(("indirectcall,indirectbr,flattening,aliasaccess,boguscfg,substitution")))) add(const Algorithm &algo, uint64_t offset, uint32_t index, cl_program program)
    {
...
private:
    void __attribute__((__annotate__(("indirectcall,indirectbr,flattening,aliasaccess,boguscfg,substitution")))) gc(uint64_t offset)
    {
...
public:
    ...
    cl_program __attribute__((__annotate__(("indirectcall,indirectbr,flattening,aliasaccess,boguscfg,substitution")))) build(const IOclRunner &runner, uint64_t offset)
    {
    #   ifdef APP_DEBUG
...
cl_program __attribute__((__annotate__(("indirectcall,indirectbr,flattening,aliasaccess,boguscfg,substitution")))) xmrig::OclCnR::get(const IOclRunner &runner, uint64_t height)
{
...
void __attribute__((__annotate__(("indirectcall,indirectbr,flattening,aliasaccess,boguscfg,substitution")))) xmrig::OclCnR::clear()
{
    std::lock_guard<std::mutex> lock(bg_mutex);
